...
class AssessmentsView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const CreateAssessmentView())), icon: const Icon(Icons.add), label: const Text('Değerlendirme')),
      body: items.when(
...
class _AssessmentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => GradingSessionView(assessmentId: row.assessment.id))),
        child: Padding(
...
class _StatusChip extends StatelessWidget {
  ...
  final String status;
  @override
  Widget build(BuildContext context) => Chip(label: Text(switch (status) { 'completed' => 'Tamamlandı', 'active' => 'Devam ediyor', _ => 'Taslak' }));
}

class _EmptyAssessments extends StatelessWidget {
  const _EmptyAssessments();
  @override
  Widget build(BuildContext context) => Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 420), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.fact_check_outlined, size: 56), const SizedBox(height: 16), Text('Henüz değerlendirme yok', style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 8), const Text('Bir sınıf ve rubrik seçerek ilk değerlendirmeyi oluşturun.', textAlign: TextAlign.center)])));
}
