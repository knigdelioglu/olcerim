...
class _WelcomeViewState extends ConsumerState<WelcomeView> {
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'Sınıfınızı aktarın, rubriğinizi oluşturun, öğrencilerinizi '
                    'değerlendirin ve raporlayın. Veriler cihazınızda kalır.',
                  ...
                  SizedBox(
                    ...
                    child: FilledButton(
                      onPressed: activeYear.value == null || loadingDemo
                          ? null
                          : () => _createFirstClass(activeYear.value!.id),
                      child: const Text('İlk sınıfımı oluştur'),
                    ),
                  ),
                  const SizedBox(height: 8),
                  TextButton.icon(
                    onPressed: loadingDemo ? null : _createDemo,
                    icon: loadingDemo
                    ...
                    label: Text(
                      loadingDemo ? 'Örnek veriler hazırlanıyor…' : 'Örnek verilerle incele',
                    ),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    'Örnek veriler tamamen sentetiktir ve istediğiniz zaman arşivlenebilir.',
  ...
  Future<void> _createFirstClass(int schoolYearId) async {
    final created = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => ClassroomFormView(initialSchoolYearId: schoolYearId),
      ),
    );
    if (created == true) {
  ...
  Future<void> _createDemo() async {
    ...
    try {
      await ref.read(demoRepositoryProvider).createDemoWorkspace();
      await ref.read(onboardingCompletedProvider.notifier).complete();
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Örnek veriler oluşturulamadı.')),
        );
      }
    } finally {
      if (mounted) setState(() => loadingDemo = false);
    }
  }
}
